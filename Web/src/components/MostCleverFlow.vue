  ...
  <div class="container">
    ...
    <div class="results">
      ...
      <div v-if="Object.keys(data).length !== 0 && data.constructor === Object">
        ...
        <ol>
          <li>
            <h1>{{ data.SECTION }}-N</h1>
            <p><b>Average Rating:</b> {{ data.AVERAGE_RATING }}</p>
          </li>
        </ol>
...
<script>
import most_clever_flows from "../../database/json/most_clever_flows.json";
export default {
  data() {
    return {
      success: false,
  ...
  methods: {
    ...
    handleClick() {
      ...
      let filtered_data = most_clever_flows.filter(
        (val) =>
