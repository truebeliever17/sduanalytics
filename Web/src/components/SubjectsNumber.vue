<template>
  <div class="container">
    <b-field class="select-field" label="Student ID">
      <b-input
        v-on:input="onChange()"
      ...
      <div v-if="Object.keys(data).length !== 0 && data.constructor === Object">
        ...
        <ol>
          <li>
            <h1>{{ data.STUD_ID }}</h1>
            <p>
              <b>Subjects:</b> {{ data.SUBJECTS_COUNT }}
            </p>
            <p>
              <b>Credits:</b> {{ data.CREDITS_COUNT }}
            </p>
          </li>
        </ol>
      </div>
...
<script>
import students_number_of_subjects from "../../database/json/students_number_of_subjects.json";
export default {
  data() {
    return {
      success: false,
  ...
  methods: {
    onChange() {
      ...
      if (
        ...
        this.selected_student.length == 0
      ) {
        this.disabled = true;
      } else {
        this.disabled = false;
    ...
    handleClick() {
      ...
      let filtered_data = students_number_of_subjects.filter(
        (val) =>
          val.YEAR == +this.selected_year &&
          val.TERM == +this.selected_term &&
          val.STUD_ID == this.selected_student.toUpperCase()
      );
