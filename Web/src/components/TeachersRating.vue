    ...
    <div class="results">
      ...
      <ol>
        <li v-for="rec in data.slice((page - 1) * 5, (page - 1) * 5 + 5)">
          <h1>{{ rec.EMP_ID }}</h1>
          <p><b>Rating:</b> {{ Math.round(+rec.AVERAGE_RATING * 10) / 10}}</p>
        </li>
      </ol>
...
<script>
import teachers_rating from "../../database/json/teachers_rating.json";
export default {
  data() {
    return {
      success: false,
  ...
  methods: {
    ...
    handleClick() {
      ...
      if (this.selected_term.length > 0 && this.selected_year.length > 0) {
        let filtered_data = teachers_rating.filter(
          (val) =>
