    ...
    <div class="results">
      ...
      <div v-if="Object.keys(data).length !== 0 && data.constructor === Object">
        ...
        <ol>
          <li>
            <h1>{{ data.STUD_ID }}</h1>
            <p><b>Term expenses:</b> {{ data.TERM_SPENT }}</p>
            <p><b>Total expenses:</b> {{ data.TOTAL_SPENT }}</p>
          </li>
        </ol>
...
<script>
import retake_expenses from "../../database/json/retake_expenses.json";
export default {
  data() {
    return {
      success: false,
  ...
  methods: {
    ...
    handleClick() {
      ...
      let filtered_data = retake_expenses.filter(
        (val) =>
