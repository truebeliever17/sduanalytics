...
popular_courses.forEach((x) => {
    if ((x['DERS_KOD'].substring(0,3) == "INF"||x['DERS_KOD'].substring(0,3) == "CSS") &&  x['YEAR'] == 2016) {
        data.y16.labels.push(x['DERS_KOD']);
        data.y16.data.push(x['REG_COUNT']);
    }
    if ((x['DERS_KOD'].substring(0,3) == "INF"||x['DERS_KOD'].substring(0,3) == "CSS") &&  x['YEAR'] == 2017) {
        data.y17.labels.push(x['DERS_KOD']);
        data.y17.data.push(x['REG_COUNT']);
    }
    if ((x['DERS_KOD'].substring(0,3) == "INF"||x['DERS_KOD'].substring(0,3) == "CSS") &&  x['YEAR'] == 2018) {
        data.y18.labels.push(x['DERS_KOD']);
        data.y18.data.push(x['REG_COUNT']);
    }

    if ((x['DERS_KOD'].substring(0,3) == "INF"||x['DERS_KOD'].substring(0,3) == "CSS") &&  x['YEAR'] == 2019) {
